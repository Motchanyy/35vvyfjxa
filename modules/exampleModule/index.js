...
class ExampleModule extends BaseModule {
	...
	_registerHooks() {
		...
		this.registerHook("displayOrderTop", async (params) => {
			return `
                ...
                <div class="module-content" style="background: #f0f0f0; padding: 15px; margin: 10px 0; border-radius: 5px;">
                    ...
                    <p>Налаштування: ${this.config.config.setting1}</p>
                </div>
            `;
		});

		// Хук для додавання CSS
		this.registerHook("displayHeader", async (params) => {
			return `
                <style>
                    .module-custom-style {
                        color: #2ecc71;
		...
		this.registerHook("displayFooter", async (params) => {
			return `
                <script>
